...
rust-test-changed:
    ...
    #
    # `asterism-importer` was on it — "one `main.rs` of clap
    # subcommands, zero test attributes" — and came off in #299, which
    # gave it `tests/import_run_e2e.rs`. That is the second time an
    # entry has gone stale, and neither time did anything here notice:
    # a member on this list is a member nothing runs, so there is no
    # run to go red.
    #
