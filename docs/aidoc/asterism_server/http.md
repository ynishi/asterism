...
second form over a single-item route, or when it talks to a team
...
one-sided, on the ground #153 stated. They are not verbs against
this server — they are a member's client talking to somebody
else's, carrying that server's session, or asking it what it offers
before there is a session to carry (#163) — so a route here would
be this process proxying a connection it does not hold and cannot
authenticate, or a discovery read against a server it is not. The
obligation is about a person not losing a verb by being on the
other surface, and a socket client that wants a team has the same
client library the desktop has for every request the desktop makes
— what it does not have is the desktop's loopback listener, which
is why a sign-in through a provider is the desktop's alone and the
changelog says so. Nothing forbids the edge —
`asterism-teams-client` is an `asterism-*` crate and #83 §4 permits
it here exactly as it permits it in `asterism-ui` — so if a headless
deployment ever needs to publish a line, this is a decision to
revisit rather than a wall.
