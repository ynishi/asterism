...
/// What the side that knows about people answers.
#[async_trait]
pub trait Actors: Send + Sync {
    /// The forge's handle on whoever this write is by.
    ///
    /// Mints one if there is not one yet — see the module docs.
    async fn resolve(&self, by: &AttributionContext) -> Result<ActorId, DomainError>;

    /// The forge's handle on this server.
