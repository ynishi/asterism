...
///
/// It lands as `Infra`, which is where the local plane's
/// `StoreFault::CorruptRow` lands too — a caller cannot fix it and
/// there is nothing to retry.
pub(crate) fn corrupt(said: String) -> DomainError {
...
pub struct LineRow {
    ...
    /// Whether it is still being worked on.
...
/// `pursuit_op` — one operation of one round, in the order it was
/// written.
#[derive(Debug, Clone)]
...
pub struct ThreadRow {
    ...
    pub kind: &'static str,
    /// Set on the anchor that names work.
...
/// The work's own row, and one `pursuit_node` row plus its
/// `pursuit_op`s for every node after the one it opened at.
...
/// off this* is the same flattening with nothing to put it in.
...
///
/// The refusals are the model's, reached through [`restore::line`]: a
/// chain that does not line up, or a table that would leave two live
/// entries under one name, fails the read.
pub fn read_line(
    head: &LineRow,
...
///
/// Nodes are put in the log's order first, by walking the parent links
/// from the node the pursuit opened at — the same walk the line side
/// does, and the reason neither table keeps a sequence beside the
/// chain.
pub fn read_pursuit(
    head: &PursuitRow,
...
pub fn read_thread(
    ...
    restore::thread(head.id, read_anchor(head)?, head.title.clone(), built).map_err(Into::into)
}
...
mod tests {
    ...
    use asterism_core::domain::forge::model::value::{ActorId, StrategyId, ThreadId};
    use chrono::TimeZone;
    ...
    /// went missing.
    ///
    /// The team's adapter reaches it no more easily than the local
    /// plane's does, which is why the case is worth carrying across
    /// with the module: a copy that kept the translation and dropped
    /// the test would leave this unasked on both planes but only
    /// answered on one.
    #[test]
