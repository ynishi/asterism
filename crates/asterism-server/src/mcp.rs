...
impl AsterismMcp {
    ...
    async fn pursuit_open(
        ...
        match self.ctx.legacy_pursuit_service.open(command, &attribution).await {
    ...
    async fn pursuit_close(
        ...
        match self.ctx.legacy_pursuit_service.close(command, &attribution).await {
    ...
    async fn pursuit_reopen(
        ...
        match self.ctx.legacy_pursuit_service.reopen(command, &attribution).await {
            Ok(event) => ok_json(&event),
            Err(err) => Ok(domain_error(err)),
        }
    }
    ...
    async fn pursuit_view(
        ...
        match self.ctx.legacy_pursuit_service.view(&params.pursuit_id).await {
            Ok(view) => ok_json(&view),
            Err(err) => Ok(domain_error(err)),
        }
    }
