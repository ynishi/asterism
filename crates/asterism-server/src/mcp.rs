...
impl AsterismMcp {
    ...
    #[tool(
        description = "Ingest one asset into the ledger. Same command shape importers POST to /asterism/assets/add. `persona_id`, `source_kind`, `locator`, `occurred_at_ms` are required. A written document is one of the things the ledger takes: point `locator` at a `.md` or `.txt` file and it lands as one asset whose words reach the body cache and the full-text search, so a phrase inside it finds it later. `asterism-import text --dir <dir>` is the same arrival for a directory of them. Declare provenance via `derived_from` (`asset:<uuid>`, `dispatch:<uuid>`, or `sidecar`) to reconnect a file that went through an outside generator. Record anything you want to *say about* the artefact — a generator's own reference, a workflow id, an edition number — in `album_meta` as `{\"key\":\"value\"}`; keys are lowercase letters, digits, `_` and `-`, and one bad entry refuses the whole ingest rather than landing an asset that answers to none of its names. Note what `album_meta` is not: a recorded identifier stays a statement, never a key — Album's own asset id is the identity, and looking rows up by a recorded value is a filter over it. Attribution is self-declared: `operator_ai` is your own slug (`claude-code`, `codex`, …) and `author_kind` is \"owner\" or \"subject\" (the latter needs `author_subject`). Omitting them records nothing — they are never filled in for you, so an absent author reads as unrecorded rather than as the owner."
    )]
    async fn asset_add(
        &self,
