...
class ForgeCatalog {
  ...
  /// second applied to the first. It is a catalog's rather than a
  /// screen's because a fold nobody can see is a fold each screen
  /// writes again slightly differently, and every rule it has to get
  /// right is somebody else's.
  ///
  /// The fold itself is `lib/forge-projection.ts`, because the shared
  /// plane asks the same question of the same two shapes and a second
  /// copy of it would diverge on the case nobody tested. What stays
  /// here is which two reads it is made of.
  get projection(): ForgeProjectedEntry[] {
    return projectWork(this.pursuit.data?.rounds ?? [], this.states.data);
  }
