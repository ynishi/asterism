//! Deterministic scene rendering for the visual-retrieval corpus (#112).
//!
//! The bench tier in [`crate::image_synth`] answers a *cost* question, so
//! its images are noise solved for an encoded-size band and nothing in them
//! means anything. A retrieval corpus answers a *relationship* question:
//! every image must have describable content so that tag suggestion and
//! text pairing have a ground truth. So this module renders from an explicit
//! [`SceneSpec`] — a gradient background plus a handful of named shapes in
//! named colors — and the spec itself *is* the ground truth: the EN/JA tags
//! and the paired caption are derived from it, never annotated by hand.
//!
//! ## Determinism
//!
//! Rendering takes no RNG: same spec ⇒ same bytes. All randomness lives in
//! the spec-drawing stream in [`crate::visual_corpus`], keyed by the corpus
//! seed, so the whole corpus is reproducible from `(seed, generator
//! version)` exactly like the bench tier.
//!
//! No file in this corpus targets the bench size band; retrieval cost is
//! not the property under measurement.
...
/// The palette is deliberately small and nameable in both languages: a tag
/// vocabulary the encoder could plausibly match, not a color histogram.
...
impl PaletteColor {
    ...
    /// Adjectival form, so a tag reads as natural Japanese ("赤い円"), not
    /// as a literal translation stub.
...
impl SceneSpec {
    /// `"<color> <shape>"` per shape, deduplicated, order-stable. This is
    /// the tag ground truth in both languages; the corpus never tags an
    /// image with anything the spec cannot show.
    ...
    /// One-sentence caption for the text-pairing half of the corpus.
...
/// between two palette endpoints passes through in-between hues — red to
/// yellow crosses orange — so an unlucky shape color can melt into the
/// background and silently falsify the tag ground truth ("orange square"
/// that no eye or encoder can see). The rim guarantees every asserted
/// shape is visible on every background the stream can produce; white is
/// not a palette color, so it never collides with a tag.
...
/// Encode to PNG with `image`'s default encoder, same as the bench tier.
...
/// A canvas of seeded per-pixel noise: outside the scene family entirely,
/// with no gradient, no shapes and no tags. The honest-failure control —
/// retrieval must be allowed to answer it with nothing.
